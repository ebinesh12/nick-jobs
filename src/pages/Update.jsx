...
import { addJobs, updateJobs } from "@/lib/api";
...
const Update = () => {
  ...
  const handleSubmit = async(e)=>{
    e.preventDefault();
    const validation = validateForm('updateJobs', formInput);
    if(!validation.valid){
      console.log(validation.message);
      return;
    }try {
      await updateJobs(formInput, id);
  ...
  return (
    <div className='py-4 mt-24 w-full flex justify-center items-start'>
        <Card className='w-96 px-8 py-4'>
          <h1 className="py-4 text-2xl font-bold text-center">Update Jobs</h1>
          <form>
            <div className="grid w-full items-center gap-4">
              ...
              <div className="flex flex-col space-y-1.5">
                <Label className='font-bold' htmlFor="name">Position</Label>
            ...
            <h3 className="py-2 text-xl font-bold text-left"> Company Details </h3>
              ...
              <Button onClick={handleSubmit} className='rounded-md w-full mt-4 px-4 py-4'>Update</Button>
              </div>
          </form>
